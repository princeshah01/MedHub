...
if (isset($_POST['login'])){
    ...
    if(empty($username)){
        $err['admin']="Enter Username";
    }elseif(empty($password)){
        $err['admin']="Enter Password";
    }
    ...
    if(count($err)==0){
        ...
        if(mysqli_num_rows($result) == 1){
           $login_done = 'Login successful! Redirecting to Admin Panel.. :) ';
  ...
  <body>
    <div class="flex justify-center items-center h-[100vh] w-[100vw] bg-[#1A202C]">
      <div class="bg-[#171a21] rounded-[10px] w-[19rem] h-[30rem] flex justify-center items-center flex-col">
        ...
        <?php endif; ?>
        <a href="login_doctor.php" class="text-[#8707ff] text-[16px] mb-4 text-right">Are you a <span class="font-extrabold hover:underline underline-offset-2">Admin</span> !</a>
